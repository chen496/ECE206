...
% Addition several basic operations on function
ramp2 = @(t, m, ad) m * ustep(t, ad);

signal = @(t, T1, T2) t .* ustep(t, 0) - (t - T1) .* ustep(t, -T1) - (t - T2) .* ustep(t, -T2) + (t - T1 - T2) .* ustep(t, -T1-T2) 
y = signal(t, 1, 2);
plot(t, y, 'k')
grid
axis([-5 5 -5 5])
